// ui.ts — HTML builders for the WebView panels (security, settings, help).
import { AnalysisResult, Violation } from "./engine";
...
export function securityHtml(data: AnalysisResult, diff: string): string {
  ...
  const errorRows = data.violations.map((v: Violation) => {
...
export function settingsHtml(s: Record<string, unknown>): string {
  const body = `
  ...
  <select id="minSeverity">
    ${["critical", "high", "medium", "low"].map((o) =>
      `<option value="${o}" ${s.minSeverity === o ? "selected" : ""}>${o}</option>`).join("")}
  </select>`;
  const script = `
  const vscode = acquireVsCodeApi();
...
export function helpHtml(): string {
  ...
  <h2>Серьёзность</h2>
  <p>${["critical", "high", "medium", "low"].map((s) =>
    `<span class="pill" style="background:${SEV_COLOR[s]}">${SEV_ICON[s]} ${s}</span>`).join(" ")}</p>
