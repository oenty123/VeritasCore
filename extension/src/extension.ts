...
import {
  analyze, clearCache, severityRank, projectDir,
  AnalysisResult, Violation, Insert,
} from "./engine";
import { securityHtml, settingsHtml, helpHtml } from "./ui";
...
const lastResult = new Map<string, AnalysisResult>(); // uri -> last analysis
...
function passesMinSeverity(sev: string): boolean {
  const min = cfg().get<string>("minSeverity") || "low";
  return severityRank(sev) >= severityRank(min);
}

// ---------------------------------------------------------------- diagnostics
function buildDiagnostics(result: AnalysisResult): vscode.Diagnostic[] {
  const out: vscode.Diagnostic[] = [];
  for (const v of result.violations) {
    if (!v.skipNoReason && !passesMinSeverity(v.severity)) continue;
    const range = new vscode.Range(
      Math.max(0, v.line - 1), Math.max(0, v.col),
      Math.max(0, (v.endLine || v.line) - 1), Math.max(v.col + 1, v.endCol));
    const d = new vscode.Diagnostic(range, v.message,
      SEV_DIAG[v.severity] ?? vscode.DiagnosticSeverity.Warning);
    d.source = SOURCE;
    d.code = v.sink;
    out.push(d);
  }
...
function worstSeverity(violations: Violation[]): string | null {
  let worst: string | null = null;
  for (const v of violations) {
    if (!passesMinSeverity(v.severity)) continue;
    if (worst === null || severityRank(v.severity) > severityRank(worst)) worst = v.severity;
  }
  return worst;
}

function updateStatus(result: AnalysisResult | null): void {
  ...
  if (shown.length === 0) {
    ...
    status.color = new vscode.ThemeColor(
      worst === "critical" || worst === "high" ? "charts.red"
...
async function refresh(doc: vscode.TextDocument, force = false): Promise<AnalysisResult | null> {
  if (doc.languageId !== "python") return null;
  const result = await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Window, title: "Nexus: анализ…" },
    () => analyze(doc, force));
  if (!result) return null;
  lastResult.set(doc.uri.toString(), result);
  diagnostics.set(doc.uri, buildDiagnostics(result));
  updateStatus(result);
  if (securityPanel) renderSecurity(doc);
  return result;
}

// ------------------------------------------------------------------- editing
function insertEdit(edit: vscode.WorkspaceEdit, uri: vscode.Uri, ins: Insert): void {
  const line = Math.max(0, ins.line - 1);
  edit.insert(uri, new vscode.Position(line, ins.mode === "inline" ? ins.col : 0), ins.text);
}

function fixedText(doc: vscode.TextDocument, violations: Violation[]): string {
  const lines = doc.getText().split(/(?<=\n)/); // keep newlines
  const inserts = violations.filter((v) => v.insert).map((v) => v.insert!) as Insert[];
...
async function applyAllFixes(doc: vscode.TextDocument): Promise<number> {
  const result = lastResult.get(doc.uri.toString()) || (await refresh(doc, true));
  if (!result) return 0;
  const inserts = result.violations.filter((v) => v.insert).map((v) => v.insert!) as Insert[];
  if (!inserts.length) return 0;
  inserts.sort((a, b) => (b.line - a.line) || (b.col - a.col));
  const edit = new vscode.WorkspaceEdit();
  for (const ins of inserts) insertEdit(edit, doc.uri, ins);
  await vscode.workspace.applyEdit(edit);
  if (!doc.isUntitled) await doc.save();   // don't pop a save dialog for untitled docs
  clearCache(doc.uri);                     // edited text invalidates the cached result
...
const codeActions: vscode.CodeActionProvider = {
  provideCodeActions(document, range, context) {
    ...
    for (const v of here) {
      const a = new vscode.CodeAction("Добавить guard (контракт проекта)",
      ...
      if (matching) a.diagnostics = [matching];
      actions.push(a);

      const skip = new vscode.CodeAction("Пропустить (# nexus: skip …)",
        vscode.CodeActionKind.QuickFix);
    ...
    if (result.violations.some((v) => v.insert)) {
      const all = new vscode.CodeAction("Применить все автофиксы в файле",
        vscode.CodeActionKind.QuickFix);
...
// ------------------------------------------------------------------ refactor
async function refactorFile(doc: vscode.TextDocument): Promise<void> {
  const result = (await refresh(doc, true));
  if (!result) return;
  const n = result.violations.filter((v) => v.insert).length;
  if (n === 0) { vscode.window.showInformationMessage("Nexus: нечего исправлять в этом файле."); return; }
  ...
  await vscode.commands.executeCommand("vscode.diff", doc.uri, previewUri,
    `Nexus: предпросмотр (${n} исправлений)`);

  const pick = await vscode.window.showInformationMessage(
    `Применить ${n} исправлений к файлу?`, "Применить", "Отмена");
  if (pick === "Применить") {
...
async function refactorWorkspace(): Promise<void> {
  const files = await vscode.workspace.findFiles("**/*.py", "**/{.git,node_modules,.venv,venv}/**", 2000);
  if (!files.length) { vscode.window.showInformationMessage("Nexus: Python-файлы не найдены."); return; }

  const plan: { doc: vscode.TextDocument; count: number }[] = [];
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Nexus: анализ workspace…", cancellable: false },
    async (progress) => {
      let i = 0;
      for (const uri of files) {
        progress.report({ message: `${++i}/${files.length}`, increment: 100 / files.length });
        const doc = await vscode.workspace.openTextDocument(uri);
        const res = await analyze(doc, true);
        const count = res ? res.violations.filter((v) => v.insert).length : 0;
        if (count) { plan.push({ doc, count }); lastResult.set(uri.toString(), res!); }
      }
    });

  const total = plan.reduce((s, p) => s + p.count, 0);
  if (total === 0) { vscode.window.showInformationMessage("Nexus: нарушений с автофиксом не найдено 🎉"); return; }

  const pick = await vscode.window.showWarningMessage(
    `Будет исправлено ${total} нарушений в ${plan.length} файлах. Применить?`,
    { modal: true }, "Применить ко всему проекту");
  if (pick !== "Применить ко всему проекту") return;

  let applied = 0;
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Nexus: применение…" },
    async (progress) => {
      for (const p of plan) {
...
async function explainFile(): Promise<void> {
  ...
  if (!result) { vscode.window.showInformationMessage("Nexus: анализ недоступен."); return; }

  const lines: string[] = ["# Nexus — локальное объяснение (COAA, без ИИ)", ""];
  const bySink = new Map<string, Violation[]>();
  for (const v of result.violations) {
  ...
  if (bySink.size === 0) {
    ...
    for (const [sink, vs] of bySink) {
      ...
      if (c && c.guard) {
        ...
        if (c.examples?.length) lines.push(`- Примеры из кодовой базы: ${c.examples.join(", ")}`);
      }
      for (const v of vs) lines.push(`- строка ${v.line}: ${v.message}`);
      lines.push("");
    }
    lines.push("## Общие рекомендации",
      "- Оборачивайте внешний ввод guard-функцией до передачи в sink.",
      "- Для команд — shlex.quote; для разбора — ast.literal_eval/json.loads; для YAML — yaml.safe_load.",
      "- Сознательные исключения помечайте `# nexus: skip <причина>`.");
  }
...
function renderSecurity(doc: vscode.TextDocument, diff = ""): void {
  if (!securityPanel) return;
  const result = lastResult.get(doc.uri.toString()) || { policy: {}, violations: [] };
  securityPanel.webview.html = securityHtml(result, diff);
}

function openSecurity(context: vscode.ExtensionContext): void {
  if (securityPanel) { securityPanel.reveal(); return; }
  securityPanel = vscode.window.createWebviewPanel(
    "nexusSecurity", "Nexus Security", vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
  ...
  if (doc) refresh(doc, true).then(() => renderSecurity(doc));
  else renderSecurity({ uri: vscode.Uri.parse("untitled:none") } as vscode.TextDocument);
}

function openSettings(context: vscode.ExtensionContext): void {
  if (settingsPanel) { settingsPanel.reveal(); return; }
  settingsPanel = vscode.window.createWebviewPanel(
    "nexusSettings", "Nexus — настройки", vscode.ViewColumn.Active, { enableScripts: true });
  ...
  settingsPanel.webview.html = settingsHtml(snapshot());
  settingsPanel.webview.onDidReceiveMessage(async (m) => {
    if (m.cmd === "set") {
      await cfg().update(m.key, m.value, vscode.ConfigurationTarget.Global);
      const ed = vscode.window.activeTextEditor;
      if (ed) { clearCache(ed.document.uri); await refresh(ed.document, true); }
    }
  }, null, context.subscriptions);
}

function openHelp(context: vscode.ExtensionContext): void {
  if (helpPanel) { helpPanel.reveal(); return; }
  helpPanel = vscode.window.createWebviewPanel(
    "nexusHelp", "Nexus — справка", vscode.ViewColumn.Active, { enableScripts: true });
  helpPanel.onDidDispose(() => (helpPanel = undefined), null, context.subscriptions);
  helpPanel.webview.html = helpHtml();
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    diagnostics, status,
    vscode.workspace.registerTextDocumentContentProvider("nexus-fixed", previewProvider),
    vscode.workspace.onDidSaveTextDocument((doc) => {
      if (cfg().get<boolean>("runOnSave")) { clearCache(doc.uri); refresh(doc, true); }
    }),
    vscode.workspace.onDidOpenTextDocument((doc) => refresh(doc)),
    vscode.window.onDidChangeActiveTextEditor((ed) => {
      if (ed) { const r = lastResult.get(ed.document.uri.toString()); updateStatus(r || null); }
      else status.hide();
    }),
    vscode.workspace.onDidCloseTextDocument((doc) => { diagnostics.delete(doc.uri); clearCache(doc.uri); }),

    vscode.commands.registerCommand("nexus.scanFile", () => {
      const ed = vscode.window.activeTextEditor; if (ed) refresh(ed.document, true);
    }),
    vscode.commands.registerCommand("nexus.openPanel", () => openSecurity(context)),
    vscode.commands.registerCommand("nexus.openSettings", () => openSettings(context)),
    vscode.commands.registerCommand("nexus.openHelp", () => openHelp(context)),
    vscode.commands.registerCommand("nexus.explainFile", explainFile),
    vscode.commands.registerCommand("nexus.applyAllFixes", async () => {
    ...
    vscode.commands.registerCommand("nexus.refactorFile", () => {
      const ed = vscode.window.activeTextEditor; if (ed) refactorFile(ed.document);
    }),
    vscode.commands.registerCommand("nexus.refactorWorkspace", refactorWorkspace),
    vscode.commands.registerCommand("nexus.skipWithReason", async (uri: vscode.Uri, line: number) => {
      ...
      await vscode.workspace.applyEdit(edit);
      await doc.save();
      clearCache(uri); await refresh(doc, true);
    }),

    vscode.languages.registerCodeActionsProvider({ language: "python" }, codeActions,
      { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }),
  );

  const ed = vscode.window.activeTextEditor;
  if (ed) refresh(ed.document, true);
}
